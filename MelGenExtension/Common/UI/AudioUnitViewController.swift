...
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    ...
    var hostingController: HostingController<MelGenExtensionMainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try MelGenExtensionAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? MelGenExtensionAudioUnit else {
				log.error("Unable to create MelGenExtensionAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(MelGenExtensionParameterSpecs.createAUParameterTree())
