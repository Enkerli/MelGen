...
enum VoiceLeadingMode: String, Codable, CaseIterable, Sendable {
...
/// that achieves it.
struct VoiceLeadingResult: Hashable, Sendable {
    ...
    var mapping: [(from: Int, to: Int)]

    static func == (lhs: VoiceLeadingResult, rhs: VoiceLeadingResult) -> Bool {
...
enum VoiceLeading {
    ...
    static func intervalClass(_ a: Int, _ b: Int) -> Int {
        let difference = abs(mod12(a) - mod12(b))
        return min(difference, 12 - difference)
    }

    static func mod12(_ value: Int) -> Int { ((value % 12) + 12) % 12 }

    /// The distinct pitch classes of a collection, in order.
    static func pitchClassSet<S: Sequence>(_ values: S) -> [Int] where S.Element == Int {
        var seen = Set<Int>()
        var result: [Int] = []
        for value in values {
            let pc = mod12(value)
    ...
    static func minimal<S: Sequence, T: Sequence>(from: S, to: T) -> VoiceLeadingResult
    ...
    static func size<S: Sequence, T: Sequence>(from: S, to: T) -> Int
    where S.Element == Int, T.Element == Int {
    ...
    /// complete.
    static func led(from previous: [Int], to targetClasses: [Int], range: ClosedRange<Int>) -> [Int] {
    ...
    static func shortestStep(from note: Int, to pitchClass: Int) -> Int {
    ...
    static func movement(from previous: [Int], to notes: [Int]) -> Int {
