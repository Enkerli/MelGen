...
struct DegreeContext: Hashable, Sendable {
    ...
    func role(ofSemitone semitone: Int) -> HarmonicRole {
...
struct DegreeHistogram: Codable, Hashable, Sendable {
    ...
    init(weights: [Double] = Array(repeating: 0, count: DegreeHistogram.size)) {
...
extension DegreeHistogram {
    ...
    static let arrivalOrder = [0, 4, 2, 6, 3, 1, 5]
    ...
    static func stack(over context: DegreeContext,
                      reach: Double = 0.55,
                      outside: Double = 0,
    ...
    static func at(_ semitones: [Int], weight: Double = 1) -> DegreeHistogram {
        var histogram = DegreeHistogram()
    ...
    static func scaleTones(of context: DegreeContext, weight: Double = 1) -> DegreeHistogram {
    ...
    /// built on.
    static func majorPentatonic(on semitone: Int, weight: Double = 1) -> DegreeHistogram {
    ...
    static func sideSlip(over context: DegreeContext,
    ...
    static func chromatic(weight: Double = 1) -> DegreeHistogram {
...
extension DegreeHistogram {
    ...
    static func mix(_ parts: [(DegreeHistogram, Double)]) -> DegreeHistogram {
    ...
    func blended(with other: DegreeHistogram, _ t: Double) -> DegreeHistogram {
        let t = max(0, min(1, t))
        return DegreeHistogram.mix([(self, 1 - t), (other, t)])
    }

    /// Multiplies the named semitones by a factor. Under 1 damps, over 1 boosts.
    func damping(_ semitones: [Int], to factor: Double) -> DegreeHistogram {
        var copy = self
        for semitone in semitones { copy[semitone] = copy[semitone] * max(0, factor) }
        return copy
    }

    func boosting(_ semitones: [Int], by factor: Double) -> DegreeHistogram {
    ...
    /// normalized yet.
    func opening(to context: DegreeContext, by amount: Double) -> DegreeHistogram {
    ...
    func emphasising(chordTonesOf context: DegreeContext, by factor: Double) -> DegreeHistogram {
...
extension DegreeHistogram {
    ...
    func probability(of semitone: Int) -> Double {
        let total = self.total
...
extension DegreeHistogram {
    ...
    func profile(over context: DegreeContext, width: Int = 24) -> [String] {
...
extension DiatonicHarmony {
    ...
    static func context(key: Int, minorness: Double) -> DegreeContext {
