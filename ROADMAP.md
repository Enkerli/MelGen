...
| F12b | **Foundation Models unavailable on the test device** | — | Found 2026-08-23 by the diagnostic in F11. `SensitiveContentAnalysisML` error 15 is the on-device **safety model's assets** being absent — "no underlying assets … for asset set com.apple.modelcatalog" — and every generation runs through that scanner, so nothing generates. A known Foundation Models fault; the remedy is Settings ▸ Apple Intelligence & Siri on Wi-Fi. Not a MelGen bug and not fixable from here. The consequence for the roadmap is larger than the bug: **every model-path item on this branch is unverified on hardware**, including model comping, the learned-style conditioning and the retry. |
