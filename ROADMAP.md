...
| U10 | **Rate the take that just went by** | S | Half done 2026-08-25, and the half that landed is the one that made the *other* half meaningful. "New take every 2 loops" now buys the time it promised: the drift re-rolls on the same cadence, so two passes really are the same performance twice rather than two different ones (see ISSUES §3). What's still open is the "previous take" affordance itself — judging `previousTakeID`, which is tracked — and whether every-2-loops should be the default rather than a setting. Worth answering after a session at 2, since the interval now behaves. |
