...
| The bass line | `BasslineGenerator` — two figure banks played as two layers at once, a pad that balances them on one axis and walks both banks sparse-to-busy on the other, a shift, a register range, eight seeds and a morph between one draw and the next. Every control draws on release, and every release is a take |
