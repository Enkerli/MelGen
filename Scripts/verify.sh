...
case "$which" in
    ...
    kernel)   run_kernel ;;
    *) echo "usage: Scripts/verify.sh [all|identity|chords|state|chunking|patterns|extraction|curation|advance|phrases|stylemodel|chain|mutation|retrieval|topics|steps|histograms|bassline|capture|comping|drift|templates|progression|analysis|midi|midifile|nextstep|docs|terminology|icon|contrast|borders|kernel]"; exit 2 ;;
esac
